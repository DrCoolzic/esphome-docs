...
- **i2c_id** (*Optional*): The I²C Bus ID
  Defaults to ``false`` 
- **model** (*Optional*): The board's model (sc16is750, or sc16is752, or auto)
  Defaults to ``auto``. When model is set to ``auto`` the model of the board will be 
  automatically detect, but you can force the model you want.
- **crystal** (*Optional*): The frequency value in Hz of the crystal used with the component.
  If you are using one of the breakout-boards described above, the frequency of the crystal 
  used on the board does not need to be specified : for a sc16is750 crystal is set to 14745600
  for a sc16is752 the crystal value is set to 3072000. But if you are building your own board
